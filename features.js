    ...
    return `<div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div class="xl:col-span-2 animate-slideUp">
            <div class="bg-black border border-zinc-900 rounded-xl overflow-hidden h-full flex flex-col">
                <div class="p-5 border-b border-zinc-900 bg-zinc-950 flex justify-between items-center"><h3 class="font-black text-white uppercase italic flex items-center"><i data-lucide="list" class="mr-2 w-4 h-4 text-zinc-500"></i> Histórico de Operações</h3><div class="flex items-center gap-2"><span class="text-xs font-bold bg-zinc-900 text-purple-400 px-2 py-1 rounded">${state.actions.length} Ações</span>${isSuperAdmin ? '<button onclick="clearAllLogs(\'ações\')" class="text-xs font-bold bg-red-900/30 text-red-400 border border-red-500/30 px-2 py-1 rounded hover:bg-red-900/50"><i data-lucide="trash-2" class="w-3 h-3 inline mr-1"></i>Limpar</button>' : ''}</div></div>
                <div class="overflow-x-auto flex-1"><table class="w-full text-left"><thead class="bg-zinc-950/50 text-zinc-500 text-[10px] font-black uppercase"><tr><th class="p-4">Ação</th><th class="p-4">Tipo</th><th class="p-4">Total</th><th class="p-4 text-purple-400">Facção</th><th class="p-4">Data</th>${isSuperAdmin ? '<th class="p-4"></th>' : ''}</tr></thead><tbody class="divide-y divide-zinc-900 text-sm">${state.actions.length === 0 ? `<tr><td colspan="${isSuperAdmin ? '6' : '5'}" class="p-8 text-center text-zinc-600 text-xs italic uppercase">Nenhuma ação registrada.</td></tr>` : state.actions.map((a, idx) => `<tr class="hover:bg-purple-900/10 group"><td class="p-4 font-bold text-white">${a.title}</td><td class="p-4"><span class="text-[10px] bg-zinc-900 px-2 py-1 rounded text-zinc-400 uppercase font-bold">${a.type}</span></td><td class="p-4 font-mono text-zinc-300">R$ ${a.total.toLocaleString('pt-BR')}</td><td class="p-4 font-mono text-purple-400 font-bold">+ R$ ${a.factionCut.toLocaleString('pt-BR')}</td><td class="p-4 text-right text-xs text-zinc-500 font-mono">${a.date}</td>${isSuperAdmin ? `<td class="p-4"><button onclick="deleteActionLog(${idx})" class="opacity-0 group-hover:opacity-100 text-red-500 hover:text-red-400 transition-all"><i data-lucide="trash-2" class="w-4 h-4"></i></button></td>` : ''}</tr>`).join('')}</tbody></table></div>
            </div>
        </div>
    </div>`;
}
...
async function submitAction() {
    ...
    logAction(
        'Ação Registrada',
        `${type}: "${title}" - R$ ${val.toLocaleString('pt-BR')} com ${selectedIds.length} membros`,
        memberNames.join(', ')
    );
...
function renderRoutes() {
    ...
    return `<div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div class="xl:col-span-1 space-y-6">
            <div class="bg-black border border-zinc-800 rounded-xl p-6 border-t-4 border-t-purple-600 animate-slideUp">
                <h3 class="text-xl font-black text-white mb-6 flex items-center uppercase italic"><i data-lucide="sprout" class="text-purple-400 mr-3 w-5 h-5"></i> Registrar Farm</h3>
                <div class="space-y-5">
                    ...
                    <button onclick="submitRoute()" class="w-full py-4 bg-purple-600 hover:bg-purple-500 text-white font-black uppercase tracking-widest rounded active:scale-95">REGISTRAR FARM</button>
                </div>
            </div>
        </div>
        <div class="xl:col-span-2 animate-slideUp">
            <div class="bg-black border border-zinc-900 rounded-xl overflow-hidden h-full flex flex-col">
                <div class="p-5 border-b border-zinc-900 bg-zinc-950 flex justify-between items-center"><h3 class="font-black text-white uppercase italic flex items-center"><i data-lucide="list" class="mr-2 w-4 h-4 text-zinc-500"></i> Hist�rico de Farm</h3><div class="flex items-center gap-2"><span class="text-xs font-bold bg-zinc-900 text-purple-400 px-2 py-1 rounded">${state.routesLog.length} Entregas</span>${isSuperAdmin ? '<button onclick="clearAllLogs(\'rotas\')" class="text-xs font-bold bg-red-900/30 text-red-400 border border-red-500/30 px-2 py-1 rounded hover:bg-red-900/50"><i data-lucide="trash-2" class="w-3 h-3 inline mr-1"></i>Limpar</button>' : ''}</div></div>
                <div class="overflow-x-auto flex-1"><table class="w-full text-left"><thead class="bg-zinc-950/50 text-zinc-500 text-[10px] font-black uppercase"><tr><th class="p-4">Membro</th><th class="p-4">Material</th><th class="p-4 text-right">Qtd</th><th class="p-4 text-right">Hora</th>${isSuperAdmin ? '<th class="p-4"></th>' : ''}</tr></thead><tbody class="divide-y divide-zinc-900 text-sm">${state.routesLog.length === 0 ? `<tr><td colspan="${isSuperAdmin ? '5' : '4'}" class="p-8 text-center text-zinc-600 text-xs italic uppercase">Nenhum farm registrado.</td></tr>` : state.routesLog.map((log, idx) => `<tr class="hover:bg-purple-900/10 group"><td class="p-4 font-bold text-white">${log.memberName} <span class="text-zinc-500 font-mono text-xs">| ID: ${log.memberGameId}</span></td><td class="p-4 text-zinc-400">${log.material}</td><td class="p-4 text-right font-mono text-purple-400 font-bold">${log.quantity || 0}</td><td class="p-4 text-right text-xs text-zinc-500 font-mono">${log.date}</td>${isSuperAdmin ? `<td class="p-4"><button onclick="deleteRouteLog(${idx})" class="opacity-0 group-hover:opacity-100 text-red-500 hover:text-red-400 transition-all"><i data-lucide="trash-2" class="w-4 h-4"></i></button></td>` : ''}</tr>`).join('')}</tbody></table></div>
            </div>
        </div>
    </div>`;
}
...
async function submitRoute() {
    ...
    state.routesLog.unshift({ memberId: memberId, memberName: member?.name || 'Desconhecido', memberGameId: member?.gameId || '?', material, quantity: quantity, date: dateStr });
    await saveFinancialData();
