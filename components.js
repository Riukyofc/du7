    ...
    return `<div class="space-y-8">
        <div class="animate-slideUp bg-black border border-zinc-800 rounded-lg p-8 relative overflow-hidden backdrop-blur-sm group hover:border-purple-600/50 transition-colors duration-500">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center relative z-10">
                <div>
                    <p class="text-zinc-400 text-xs font-black uppercase tracking-[0.3em] mb-2">Caixa da Organização</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-5 animate-slideUp">
                ...
                ${recentTransactions.length > 0 ? `<div class="space-y-3">${recentTransactions.map(t => `
                    <div class="bg-zinc-950/50 border border-zinc-900 rounded p-4 flex items-center justify-between hover:border-purple-600/50 transition-all">
...
function renderMembers() {
    return `<div class="space-y-8">
        <div class="flex justify-between items-center animate-slideUp"><h2 class="text-3xl font-black text-white tracking-tighter uppercase italic">Membros <span class="text-purple-500">ROCINHA</span></h2></div>
        <div class="bg-black border border-zinc-900 rounded-xl overflow-hidden shadow-2xl animate-slideUp">
            <table class="w-full text-left">
                <thead><tr class="bg-zinc-950 border-b border-zinc-900 text-zinc-500 text-xs font-black uppercase tracking-[0.2em]"><th class="p-6">Membro</th><th class="p-6">Cargo</th><th class="p-6">Status</th><th class="p-6">Financeiro</th><th class="p-6">Ações</th><th class="p-6">Rotas</th></tr></thead>
                <tbody class="divide-y divide-zinc-900/50">
                    ${state.members.length === 0 ? `<tr><td colspan="6" class="p-8 text-center text-zinc-600 text-sm italic">Nenhum membro registrado.</td></tr>` : state.members.map(m => `
                        <tr class="hover:bg-purple-900/10 transition-colors group">
                            ...
                            <td class="p-6"><span class="text-xs px-3 py-1.5 rounded border bg-black font-bold uppercase ${m.role === 'Líder' || m.role === 'Dono' ? 'text-purple-400 border-purple-500/30' : 'text-zinc-400 border-zinc-800'}">${m.role || 'Vapor'}</span></td>
                            ...
                            <td class="p-6 text-purple-400 font-mono font-bold">R$ ${(m.financial || 0).toLocaleString('pt-BR')}</td>
